# Examples
{% prettylink link="https://github.com/acrylplatform/ride-examples" %}RIDE Examples on Github{% endprettylink %}
...
A dApp wallet is implemented where you can send payment as ACRYL and save them in the wallet via the callable _**deposit function**_ or you can take them back via the callable _**withdraw function**_.

_**As a first step**_, let's write a function which gets how much ACRYL did the **contract issuer** give to the contract. let's name this function **getBalance\(address\) :Int. **
...
Now we need to define a **deposit function**\(as Callable function\) where the user can send payment as ACRYL and save them in the wallet.
...
func deposit() = {
  ...
  let currentBalance = getBalance(caller) #how much ACRYL did the contract issuer give to the contract.
  ...
  if (payment.assetId != unit) then throw("This wallet cannot hold assets other than ACRYL")
  else {
...
Finally we need to write the **withdraw function** \(Callable function\) where the user can take ACRYL back.
...
In this Example, let's suppose that we need to implement a mechanism to ensure that funds in an account will be divided equally between two specific addresses Alice and Bob. To each of them we will send exactly \(\(balance - fee\) / 2\) ACRYL.
...
@Callable(i)
func divideAmount() = {
# calculate the amount of ACRYL that will be transferred to Alice and Bob
    let transferAmount = acrylBalance(this) / 2
...
You can execute the functions by using [IDE](https://ide.acrylplatform.com/) console. Also it's available in the JS library \([acryl-transactions](/acryl-api-and-sdk/client-libraries/acryl-transactions.md)\).
