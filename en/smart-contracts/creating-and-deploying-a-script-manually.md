...
In this example, we're going to create and deploy a simple 2 of 2 MultiSig example without using neither `Acryl Client libraries` nor `API libraries`.

**Example Assumptions:**

1. We assume that we have our own Acryl node \(if you do not have Acryl node, you can implement the example using one of [Acryl Libraries](/acryl-api-and-sdk/client-libraries.md) as shown in this [video tutorial](https://www.youtube.com/watch?v=o2msjSo0y0o&t=21s) using Acryl Java Library\).
2. We want to set up a script for a [generated](/acryl-node/node-api/address.md#post-addresses) account in order to implement the **smart account **idea.
...
1. Use our [IDE](https://ide.acrylplatform.com) to write your script \(you can find some script examples there\)
...
2. Send it to [/transactions/sign](/acryl-node/node-api/transactions.md#post-transactionssign):
...
3. Then we [broadcast](/acryl-node/node-api/transactions.md#post-transactionsbroadcast) a prepared request:
...
## 2. Example of transferring Acryl with a bad request
...
## 2.1 Example of transferring Acryl with a successful request
...
1. Alice signs this request by her private key through [POST /transactions/sign/{signerAddress}](/acryl-node/node-api/transactions.md#post-transactionssignsigneraddress)
...
3. Bob signs it by his private key by same method [POST /transactions/sign/{signerAddress}](/acryl-node/node-api/transactions.md#post-transactionssignsigneraddress)
...
5. Bob [broadcasts](/acryl-node/node-api/transactions.md#post-transactionsbroadcast) the transaction
