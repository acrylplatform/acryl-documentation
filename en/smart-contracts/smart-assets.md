...
_**Smart assets**_ are unique virtual currency tokens that may represent a tangible real-world asset, or a non-tangible ownership that can be purchased, sold, or exchanged as _**defined by the rules of a script**_ on the Acryl blockchain network.
...
The transaction fee is calculated in the same way as for [smart accounts](../smart-contracts/smart-accounts.md): for each time the script is called, [total transaction’s fee](../acryl-environment/acryl-protocol/transactions-fees.md) increases by 0.004 ACRYL.

**Examples:**

* If user will transfer smart assets from scripted account the final fee is 0.009.acryl
* x for a TransferTransaction of a Smart Asset from a Smart Account would be 0.001+0.004+0.004=0.009 ACRYL
...
If an ExchangeTransaction's sender \(the Matcher or any other account\) has a script then the total fee for the transaction is increased by 0.004 acryl.

## Validation

A smart asset’s script validates any of [_**the following transaction**_](../acryl-environment/acryl-protocol/transactions-structure.md) types with the asset:
...
You can create a smartAsset via [IssueTransaction\(Version2\)](../acryl-environment/acryl-protocol/transactions-structure.md) and specify the script in this transaction.

Here’s an example of JSON for [IssueTransaction\(Version2\)](https://nodes.acrylnodes.com/transactions/info/FTQvw9zdYirRksUFCKDvor3hiu2NiUjXEPTDEcircqti):
...
A smart asset’s script can be changed via [_**SetAssetScriptTransaction**_](/blockchain/binary-format/transaction-binary-format/set-asset-script-transaction-binary-format.md) \([fee](../technical-details/transactions-fees.md) on changing is equal to 1 ACRYL\).

Only the issuer can change the asset's script.

## Examples of Scripts for Smart Assets

You can find an example of _**SetAssetScript **\_transaction on _**testnet**\_ in the following [transactions examples](../acryl-node/node-api/example-transactions.md).
...
As a result, for the Orders placement Alice pays fee=0.011 acryl to the matcher, Bob pays fee=0.011 acryl to the matcher. When the ExchangeTransaction is being put into the blockchain, the fee payed by the matcher is 0.011+0.004\(because of the matcher’s script\)=0.015 acryl.
