...
In order to setup an Smart Account, The account needs to issue [SetScriptTransaction](https://acrylplatform.github.io/acryl-transactions/globals.html#setscript) which contains the predicate. Upon success, every outgoing transaction will be validated not by the default mechanism of signature validation, but according to the predicate logic.  
`AccountScript` can be changed or cleared if the script installed allows the new [SetScriptTransaction](https://acrylplatform.github.io/acryl-transactions/globals.html#setscript) to process.  
...
**Note.** [SetScriptTransaction](https://acrylplatform.github.io/acryl-transactions/globals.html#setscript) sets the script which verifies all outgoing transactions. The set script can be changed by another [SetScriptTransaction ](https://acrylplatform.github.io/acryl-transactions/globals.html#setscript)call unless it’s prohibited by a previous set script.
...
* The fixed cost for each scripted unit is equal to **400000 **_**acrylettes**_ \(Acryl coins, 100000000 acrylettes = 1 Acryl\).

**Examples:**

* If transaction's usual cost is 0.001 ACRYL, in case of smart scripting exactly the same transaction will be cost 0.001 + 0.004 ACRYL.
* Set Alias for an account without a script would cost 0.001 ACRYL, whereas for a Smart Account it would be 0.001+0.004=0.005 ACRYL.
...
Acryl also added trader’s script check in Matcher. When it receives an order from a smart account, it executes the script for the order.
...
#### 2. Buy back custom asset on specified price in ACRYL:
