...
Acryl provides a decentralized exchange \(DEX\), which allows trading different assets back and forth between users, as a traditional exchange, with stronger security guarantees to end users due to its decentralized nature. An opportunity of creating some new assets, based on Acryl, allows early trading of a crowdfunding stake, that provides liquidity for tokens. For this purpose, tokens should be sold in public locations, where buyers and sellers may post orders.
...
2. If there is a counter-order that matches with the submitted order, then the order execution is performed. That means the counter-order is removed from order book and the matcher creates exchange transaction, signs it by matcher's private key and is sent to the Acryl network for including in the blockchain.
...
The fix full transaction fee now is equal to **0.003 acryl for one order**, whether buying or selling, regardless of the amount of the future deal. The exchange transaction contains two separate fields for Matcher's fee, which goes from buyer's order and seller's order. An order can be fully executed by some transaction, in this case, all matcher fee from it is included in that transaction.
...
There are 3 different orders \(Figure 2\): two buy orders and one sell. For each full order, a user has to pay exactly **0.003 acryl** of a fee, and this fee will be written off as the order is executed. In our example:

* the Order1 is fully matched with a 70% part of Order3 by Transaction1 and matcher's fee for this transaction is equal to 0.003 + 0.0021 - 0.003 = 0.0021 acryl since Matcher pay to miners transaction fee which is also equal to 0.003 Acryl.
* The 50% of Order2 matches with 30% part of Order3 by Transaction2 and matcher's fee for this transaction is equal to 0.0009 + 0.0015 - 0.003 = -0.0006 acryl.

Thus, the fee that the matcher gets from users for these transactions is**0.0021 - 0.0006  = 0.0015 acryl**. And the fee that the matcher pays to miners is**0.006acryl**.
...
| :--- | :--- | :--- | :--- |
| last month | 681.42336675 acryl | 569.721 acryl | 16.39% |
| all time | 3476.01418346 acryl | 2824.771 acryl | 18.74% |
...
* [ACRYL](../development-and-api/node-api/address.md#get-addressesbalanceaddress);
...
For example, if you are transferring `ACRYL` and sending a [data transaction](/blockchain/transaction-type/data-transaction.md)  
and these transactions haven't yet forged, `spendings_of_ACRYL_in_transactions_in_utx_pool` will be `amount_of_transferred_acryl + transfer_fee + data_transaction_fee`.
...
Note, for `ACRYL` order's fees are included in reserved balance.

As you know, all orders in `DEX` requires `ACRYL` as the fee. Because of this, there is an exception for the rule we spoke above.  
If you buy `ACRYL` by other asset, reserved balance in acryl in this order will be: `max(fee - amount_of_received_acryl_in_this_order, 0)`.  
So, you can buy `ACRYL` for `BTC` even if you have no `ACRYL`.

# 5. Installing DEX

* Download the Acryl client from our official website,
  [www.acrylplatform.com](http://www.acrylplatform.com/)
  , or use the webwallet, available at
  [client.acrylplatform.com](https://dex.acrylplatform.com/)
* [Deposit your bitcoins](/acryl-client/transfers-and-gateways/bitcoin-transfers.md) or any [supported coins and tokens](/en/acryl-client/wallet-management.md) into the wallet and [start trading using the Acryl DEX](/acryl-client/acryl-dex.md).

# 6. Installing your Own Matcher

* Users can install their own matcher by installing the Acryl software and enabling the matching functionality.
