...
If you want to authorize a user in your service by means of his Acryl account, here's the solution. In general, you should redirect the user to the official Acryl Client \([https://dex.acrylplatform.com/](https://dex.acrylplatform.com/)\) with certain query parameters including some arbitrary data for him to sign.
...
1. You add the Acryl Auth widget to your site.
2. A user stumbles upon your site and wants to log in using his Acryl account.
3. He clicks the widget button and gets redirected to the official Acryl Client, along with some random data from the widget.
...
If the user interrupts the process, he stays on the Acryl Client page.
...
[Example](https://dex.acrylplatform.com#gateway/auth?r=https://example.com&n=Service%20Name&d=0123456789abc&i=/img/logo.png&success=/acrylAuth): `https://dex.acrylplatform.com#gateway/auth?r=https://example.com&n=Service%20Name&d=0123456789abc&i=/img/logo.png&success=/acrylAuth`.

Basic path is `https://dex.acrylplatform.com#gateway/auth`. Then the query parameters go.
...
`?a=base58EncodedAddress` — user's Acryl address.

### How to check signature validity

You can use the [@acryl/signature-generator](https://www.npmjs.com/package/@acryl/signature-generator) npm package.

Signed data consists of three objects `Prefix string` + `URL host` + `Provided Data`
Signature is taken from the data in the following order: a `AcrylWalletAuthentication` string, then a string with your host parameter value, then a string with your data parameter value.
...
#### Js example code
```javascript
const sg = require('@acryl/signature-generator')
...
function authValidate(data, sign, publicKey) {
  const prefix = 'AcrylWalletAuthentication'
...
def signed_data(host, data):
    prefix = 'AcrylWalletAuthentication'
...
/*
 * Requires AcrylKit by deemru
 * https://github.com/deemru/AcrylKit
 */
