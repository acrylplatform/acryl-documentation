...
Build from scratch, here you will need to [_**run the node**_](/acryl-node/how-to-install-a-node/how-to-install-a-node.md), then it will receive all the needed information from the other nodes.
...
{% prettylink link="../acryl-node/options-for-getting-actual-blockchain/import-from-the-blockchain.md" %} Import From Blockchain {% endprettylink %}
...
[_**Downloading state**_](/acryl-node/options-for-getting-actual-blockchain/state-downloading-and-applying.md), approximately the size of the State is around 23 GB \(**in November 2018**\).

In this case, the longest operation is to download the archive. Then it is enough to unpack the archive files to the /var/lib/acryl/data directory and run the node.  The remaining blocks will be synchronized during normal operation.
