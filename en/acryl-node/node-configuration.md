...
> After upgrading to version 1.0.2 please note if your `/etc/acryl/acryl.conf` was originally copied from a template, you may need to assure that acryl.directory points to the correct directory. If this option doesn't exist in the config, default directory `/var/lib/acryl` and `/var/lib/acryl-testnet` will be used for mainnet and testnet, respectively

## Configuration Format

The configuration system of Acryl Node uses HOCON format. HOCON stands for Human-Optimized Config Object Notation. The complete description of HOCON could be found in the [Official HOCON documentation](https://github.com/typesafehub/config/blob/master/HOCON.md). The advantages of HOCON are simple syntax and ability to use comments.

## Default Configs and Overrides

### Default Configuration Embedded into JAR

Complete default Acryl Node configuration file which is embedded into jar-file can be found here: https://github.com/acrylplatform/Acryl/blob/master/node/src/main/resources/application.conf
...
* https://github.com/acrylplatform/Acryl/blob/master/node/acryl-mainnet.conf
* https://github.com/acrylplatform/Acryl/blob/master/node/acryl-testnet.conf

### Overriding parameters when running JAR-file

If you run JAR file it's recommended to override default parameters by passing a path to config file as the command line parameter then starting Acryl Node application.

```java -jar acryl-all-0.13.3.jar acryl.conf```

Typically this file should contain you node's unique characteristics (ip, name, keys, etc...) and network-specific parameters similar to acryl-mainnet or acryl-testnet configs from previous sections (files shipped with DEB packages).


## Configuration Sections

### Acryl configuration section

Root configuration section `acryl` holds essential application parameters and other configuration subsections.
...
**Note:** If you want to change acryl directory in Ubuntu packages you should change it using `-J-Dacryl.directory=path` in `/etc/acryl/application.ini` and `/lib/systemd/system/acryl.service`. You can override any JVM start parameter in `acryl.service`, it has priority.
...
**Note:** Make sure the defined directory has a correct owner set: `acryl` for mainnet or `acryl-testnet` for testnet.
...
Use `file` parameter to set the location of peers database. In this database node stores lists of known and blacklisted peers. By default, the path is resolved with regard to base `directory` from `acryl` section.
...
Using parameter `bind-address` you can set the IP address of local network interface on which Acryl Node will accept incoming connections. By default, node binds to `0.0.0.0` that means that it will listen on all available network adapters.

Use `port` parameter to set the network port number to which other Acryl nodes will connect. Check that the port is reachable from outside otherwise, your node will connect to P2P network only using outgoing connections. If this the port is taken by other application, your node won’t start.
...
In `upnp` section you can set the UPnP settings. Actually, those settings are useful only if you ran your Acryl node on the home network where the node could ask your router to establish a tunnel. By default, this functionality is disabled. Use`enable`parameter of`upnp`to enable this functionality.
...
In `wallet` section you can configure wallet built in Acryl node.
...
Using `min-order-fee` parameter it’s possible to set the minimum required fee to process orders. For now, matcher accepts fee only in ACRYL. The fee should be given in minimal units \(ACRYLETTES\).
...
To specify Acryl as a side of a pair use special asset ID "ACRYL".
...
predefined-pairs = [
     {amountAsset = "ACRYL", priceAsset = "Fmg13HEHJHuZYbtJq8Da8wifJENq8uBxDuWoP9pVe2Qe"},
     {amountAsset = "ACRYL", priceAsset = "HyFJ3rrq5m7FxdkWtQXkZrDat1F7LjVVGfpSkUuEXQHj"},
     {amountAsset = "ACRYL", priceAsset = "2xnE3EdpqXtFgCP156qt1AbyjpqdZ5jGjWo3CwTawcux"},
     {amountAsset = "ACRYL", priceAsset = "6pmDivReTLikwYqQtJTv6dTcE59knriaodB3AK8T9cF8"},
...
In `micro-block-synchronizer` subsection you could tune various parameters of Acryl-NG protocol.
