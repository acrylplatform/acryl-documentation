...
Balances for all assets that the given account ever had \(besides ACRYL\).
...
"sponsorBalance" - sponsor balance in acryl
```
...
"fee" - Transaction fee for Asset issue, min = 100000000 (1ACRYL).
...
* Signed transfer request:
    ...
    ```
    "assetId" [optional] - Asset ID to transfer or omit that param when transfer ACRYL, Base58-encoded
    "senderPublicKey" - Sender account's public key, Base58-encoded
    "recipient" - Recipient account's address, Base58-encoded
    "fee" - Transaction fee for Asset transfer, min = 100000 (ACRYLETTEs)
    "feeAssetId" [optional] - Asset ID of transaction fee. ACRYL by default, if empty or absent
    "amount" - amount of asset'lets (or acrylettes) to transfer
...
* Signed versioned transfer request:
    ...
    "recipient" - Recipient account's address, Base58-encoded
    "fee" - Transaction fee for Asset transfer, min = 100000 (ACRYLETTEs)
    "amount" - Amount of acrylettes to transfer
    "attachment" - Arbitrary additional data included in transaction, max length is 140 bytes, Base58-encoded
    "timestamp" - Transaction timestamp
...
* Signed transfer response:
    ...
    "senderPublicKey" - Sender account's public key, Base58-encoded
    "recipient" - Recipient account's address, Base58-encoded
    "feeAsset" - Asset ID of transaction fee, currently is null, i.e. ACRYL
