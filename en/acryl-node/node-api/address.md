...
```
"address" - account's address in Base58 format
"Regular" — that's how much Acryl you have, including those you leased;
"Available" — the same as regular only without Acryl you leased;
"Effective" — available plus those Acryl which is leased to you;
...
Get account balance in ACRYL in {address}:
...
Get account balance in ACRYL by {address} after {confirmations} from now:
