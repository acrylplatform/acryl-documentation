...
Acryl предлагает децентрализованную биржу \(DEX\), которая позволяет торговать различными ассетами между пользователями, как традиционная биржа, но с более надежными гарантиями безопасности для конечных пользователей из-за децентрализованного характера. Возможность создания новых ассетов, на базе Acryl, позволяет организовывать этап ранней торговли с краудфандингом, что обеспечивает ликвидность токенов. Для этой цели токены должны продаваться в общем доступе.
...
2. Если есть встречный ордер, соответствующий заявке, то заявка исполняется. Это означает, что встречный ордер достаётся из стакана, матчер создает биржевую транзакцию, подписывает ее своим закрытым ключом и отправляет в сеть Acryl для включения в блокчейн.
...
Полная фиксированная комиссия за транзакцию в настоящий момент равна **0,003 acryl за заказ**, будь то покупка или продажа, независимо от суммы будущей сделки. Биржевая сделка содержит два отдельных поля для комиссии матчера: от заявки покупателя и заявки продавца. Ордер может быть полностью выполнен какой-либо транзакцией, в этом случае вся сумма комиссий от него включена в эту транзакцию.
...
Пусть есть 3 разных ордера \(Рисунок 2\): два на покупку и один на продажу. За каждый полный ордер, пользователь должен a заплатить ровно **0.003 acryl** комиссии, и эта комиссия будет списана только тогда, когда ордер исполнится. Например:

* `Order1` полностью исполняется с 70% частью Order3 в `Transaction1` и комиссия матчера для этой транзакции составляет `0.003 + 0.0021 - 0.003 = 0.0021 Acryl` так как матчер платит майнерам комиссию, также равную 0.003 Acryl.
* 50% от `Order2` исполняется с 30% частью `Order3` в `Transaction2` и комиссия матчера за эту транзакцию равна `0.0009 + 0.0015 - 0.003 = -0.0006 Acryl`.

Таким образом, комиссия, которую матчер получает от пользователей за их транзакции равна **0.0021 - 0.0006  = 0.0015 acryl**. Комиссия, которую матчер платит майнерам равна **0.006 acryl**.
...
| :--- | :--- | :--- | :--- |
| последний месяц | 681.42336675 acryl | 569.721 acryl | 16.39% |
| всё время | 3476.01418346 acryl | 2824.771 acryl | 18.74% |
...
* Скачать Acryl клиент с официального сайта,
  [www.acrylplatform.com](http://www.acrylplatform.com/)
  , или использовать онлайн кошелёк, доступный здесь
  [beta.acrylplatform.com](https://dex.acrylplatform.com/)
* [Поместите свои биткоины](/acryl-client/transfers-and-gateways/bitcoin-transfers.md) или любые другие [поддерживаемые монеты и токены](/acryl-client/wallet-management.md) в свой кошелёк [и начните торговать на Acryl DEX](/acryl-client/acryl-dex/start-trading-using-the-acryl-dex.md).

# 5. Установка своего собственного матчера 

* Пользователи могут установить их собственный матчер, установив ПО Acryl и сделав доступной функциональность матчера.
